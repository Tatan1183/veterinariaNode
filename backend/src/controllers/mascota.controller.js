...
const toMascotaDTO = (mascota) => {
  ...
  if (mascota.cliente && typeof mascota.cliente === "object") {
    ...
    // Si cliente es solo un ID (en creación/actualización antes de populate)
...
export const getMascotas = async (req, res) => {
  try {
    const mascotas = await Mascota.find().populate(
      "cliente",
      "nombre apellido"
    ); // Popula nombre y apellido del cliente
...
//Mostras Mascotas por ID
export const getMascotaById = async (req, res) => {
  try {
    const mascota = await Mascota.findById(req.params.id).populate(
      "cliente",
      "nombre apellido"
    );
    if (!mascota)
...
export const createMascota = async (req, res) => {
  const { nombre, especie, raza, fechaNacimiento, imagen, clienteId } =
    req.body;
  try {
    ...
    savedMascota = await Mascota.findById(savedMascota._id).populate(
      "cliente",
      "nombre apellido"
    ); // Popula después de guardar
...
export const updateMascota = async (req, res) => {
  ...
  try {
    ...
    let mascota = await Mascota.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
