...
export const createCita = async (req, res) => {
    const { mascotaId, veterinarioId, servicioId, fechaHora, estado, notas } = req.body;
    try {
        ...
        if (!mascota) return res.status(400).json({ message: 'Mascota no encontrada.' });
        if (!veterinario) return res.status(400).json({ message: 'Veterinario no encontrado.' });
        if (!servicio) return res.status(400).json({ message: 'Servicio no encontrado.' });
...
export const updateCita = async (req, res) => {
    ...
    try {
        ...
        let cita = await Cita.findByIdAndUpdate(req.params.id, updateData, { new: true })
            .populate('mascota', 'nombre')
            .populate('veterinario', 'nombre apellido')
            .populate('servicio', 'nombre');
